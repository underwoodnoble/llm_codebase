...
@torch.no_grad()
def llm_inference(dataset: List, tokenizer: PreTrainedTokenizer, model: PreTrainedModel, batch_size: int, generation_config: GenerationConfig, save_path: str):
    if 'prompt' in dataset[0]:
        prompts = [data['prompt'] for data in dataset]
    elif 'query' in dataset[0]:
        prompts = [data['query'] for data in dataset]
    else: 
        raise ValueError('wrong data!')
    
        
    for i in range(0, len(prompts[:10]), batch_size):
        encoding = tokenizer(prompts[i:i+batch_size], return_tensors='pt', padding=True, truncation=True)
        ret = model.generate(encoding['input_ids'].to(model.device), generation_config)
        responses = tokenizer.batch_decode(ret)
        print(responses)
        with open(save_path, 'a') as f:
            for i, prompt in enumerate(prompts[i:i+batch_size]):
                prompt_responses = responses[i*generation_config.num_return_sequences:(i+1)*generation_config.num_return_sequences]
                f.write({
                    'prompt': prompt,
...
def main(args):
    ...
    if args.task_type == 'rm':
        ...
        model.eval()
        llm_inference(dataset, tokenizer, model, args.batch_size, generation_config, args.save_path)
