...
class CustomArguments(transformers.TrainingArguments):
    ...
    data_path: str = field(default=None, metadata={"help": "train datasets paths."})

    eval_data_dir: str = field(default=None, metadata={"help": "the directory to load evaluation datasets."})
    eval_data_path: str = field(default=None, metadata={"help": "evaluation datasets path."})
    ...
    model_max_length: Optional[str] = field(default=512, metadata={"help": "max num of tokens the model can handle."})
    ...
    def __post_init__(self):
        ...
        if self.data_dir is None and self.data_path is None:
            raise ValueError(f"One of data_dir and data_path must be set.")
        if self.data_dir is not None and self.data_path is not None:
            raise ValueError(f"Only one of data_dir and data_path should be set.")
        if self.eval_data_dir is None and self.eval_data_path is None:
            raise ValueError(f"One of eval_dir and eval_data_path must be set.")
        if self.eval_data_dir is not None and self.eval_data_path is not None:
            raise ValueError(f"Only one of eval_dir and eval_data_path should be set.")

        valid_model_types = ['bert']
