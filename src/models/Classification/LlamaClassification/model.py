...
class LlamaClassifer(LlamaPreTrainedModel):
    config_class = LlamaClassiferConfig
    def __init__(self, config: LlamaClassiferConfig, model_path=None):
        ...
        self.cls_heads = nn.ModuleDict(
            {key: nn.Linear(config.hidden_size, value) for key, value in config.cls_label_nums.items()}
        )
    ...
    def forward(
        ...
        labels_dict: Optional[torch.FloatTensor] = None,
        ...
        if self.config.pad_token_id is None:
            ...
            if input_ids is not None:
                sequence_lengthes = (torch.ne(input_ids, self.config.pad_token_id).sum(dim=-1).to(hidden_states.device))
            else:
                sequence_lengthes = -1
        ...
        if pooling_type in ['last', 'eos']:
            ...
            if padding_side == 'right':
                pooled_hidden_state = hidden_states[torch.arange(batch_size, device=hidden_states.device), sequence_lengthes - offset]
            else:
        ...
        output_logits_dict: Dict[str, torch.Tensor] = {key: head_layer(pooled_hidden_state) for key, head_layer in self.cls_heads.items()}
